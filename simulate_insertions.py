...
def main():
    ...
    nuclear_positions = get_random_sequence_positions(arguments["destiny"], arguments["insertion_length"])
    ...
    for indentity_range in IDENTITIES:
        ...
        with open(arguments["input_sequence"]) as input_fhand:
            sequences = get_sequences(input_fhand, insertion_positions, freq1, freq2, 
                                      arguments["insertion_length"], out_fpath)
        with open(arguments["destiny"]) as destiny_fhand:
            insertions = insert_into_nucleus(destiny_fhand, sequences, nuclear_positions, out_fpath)
